...
class Trainer:
    ...
    def validate(self):
        ...
        # 聚合 evaluator 统计
        if self.distributed:
            for key, meter in self.evaluator.metrics.items():
                # meter.sum 是 tensor，meter.count 是标量
                sum_tensor = meter.sum if torch.is_tensor(meter.sum) else torch.tensor(meter.sum, device=self.device, dtype=torch.float64)
                cnt_tensor = torch.tensor(float(meter.count), device=sum_tensor.device)
                packed = torch.stack([sum_tensor.to(torch.float64), cnt_tensor])
                dist.all_reduce(packed, op=dist.ReduceOp.SUM)
                meter.sum = packed[0]
                meter.count = int(packed[1].item())
    ...
    def process_batch(self, inputs):
        ...
        if self.eq_loss_weight > 0 and self.model.training:
            loss_eq = self.compute_eq_loss(x, pred.unsqueeze(2))
